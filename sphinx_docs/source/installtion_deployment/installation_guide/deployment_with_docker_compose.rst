...
This guide will help you deploy Flowpilot in a production environment using Docker Compose.
...
- **Database Configuration**:
  ...
  - ``POSTGRES_USER``: PostgreSQL database username (default: flowpilot)
  - ``POSTGRES_PASSWORD``: PostgreSQL database password (**must be changed**)
  - ``POSTGRES_DB``: Database name (default: flowpilot)
...
2. Start all services:
   ...
   - **flowpilot-redis**: Redis service (for generating ticket serial numbers and Celery async tasks)
   - **flowpilot-pg**: PostgreSQL database service
   - **flowpilot-backend**: Flowpilot backend service (Django + Gunicorn)
   - **flowpilot-ui**: Flowpilot frontend service (Nginx)
   - **flowpilot-task**: Celery task processing service
...
4. View service logs (optional):
   ...
   .. code-block:: bash
      ...
      docker-compose logs -f flowpilot-backend

Accessing the Application
-------------------------

After the services are started, you can access Flowpilot through your browser:

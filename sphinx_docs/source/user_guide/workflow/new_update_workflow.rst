Create Or Update Workflow
=========================

This guide is intended for new admin or workflow admin who want to configure workflows.

Create New Workflow
-------------------

1. Login to FlowPilot
2. Click the "Workflow Management" button in the left sidebar
3. Click the "New Workflow" button

   .. figure:: ../../images/new_workflow.png
      :width: 100%
      :align: center
      :alt: Workflow List

4. Fill in the workflow basic information

   .. figure:: ../../images/fill_workflow_basic_info.png
      :width: 100%
      :align: center
      :alt: Fill Workflow Basic Info

5. Finish form design

   .. figure:: ../../images/fill_workflow_form_design.png
      :width: 100%
      :align: center
      :alt: Fill Workflow Form Design

6. Finish process design

   .. figure:: ../../images/fill_workflow_process_design01.png
      :width: 100%
      :align: center
      :alt: Fill Workflow Process Design

   .. figure:: ../../images/fill_workflow_process_design02.png
      :width: 100%
      :align: center
      :alt: Fill Workflow Process Design

   .. figure:: ../../images/fill_workflow_process_design03.png
      :width: 100%
      :align: center
      :alt: Fill Workflow Process Design

7. Click the "RELEASE" button, then set the workflow version and click the "CONFIRM" button


Update Workflow
---------------

1. Login to FlowPilot
2. Click the "Workflow Management" menu in the left sidebar
3. Find the workflow you want to modify and click the EDIT button in the action column on the right
4. In the version list popup, choose the version to edit and click the workflow edit button on the right to open the workflow edit page
5. After editing the workflow, click the "RELEASE" button, then set the workflow version and click the "CONFIRM" button

.. note::
    FlowPilot automatically checks whether your changes are compatible with previous versions and will remind you. You can save as a new version or keep the current version. If you save as a new version, you can adjust the version type on the workflow version management page.

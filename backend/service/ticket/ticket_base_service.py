...
class TicketBaseService(BaseService):
    ...
    def new_ticket(cls, tenant_id: str, app_name: str, operator_id: str,  request_data_dict: dict)->int:
        ...
        for next_ticket_node_result0 in next_ticket_node_result_list:
            if not next_ticket_node_result0.get('keep_previous', False):
                transaction.on_commit(lambda: flow_notification_task.apply_async(args=[tenant_id, ticket_id, next_ticket_node_result0.get("id")], queue='flowpilot'))
        ...
        for next_ticket_node_result in next_ticket_node_result_list:
            if next_ticket_node_result.get("type") == "hook":
                ...
                transaction.on_commit(lambda: flow_hook_task.apply_async(args=[tenant_id, ticket_id, next_ticket_node_result.get("id")], queue='flowpilot'))

        # event hook
        from tasks import flow_action_hook_task
        transaction.on_commit(lambda: flow_action_hook_task.apply_async(args=[tenant_id, ticket_id, "create"], queue='flowpilot'))
    ...
    def withdraw_ticket(cls, tenant_id: str, ticket_id: str, operator_id: str, request_data_dict: dict) -> bool:
        ...
        # withdraw hook
        from tasks import flow_action_hook_task
        transaction.on_commit(lambda: flow_action_hook_task.apply_async(args=[tenant_id, ticket_id, "withdraw"], queue='flowpilot'))
        return True
    @classmethod
    ...
    def forward_ticket(cls, tenant_id: str, ticket_id: str, operator_id: str, node_id: str, source_assignee_id: str, target_assignee_id: str, commment:str) -> bool:
        ...
        # send notification
        from tasks import flow_notification_task
        flow_notification_task.apply_async(args=[tenant_id, ticket_id, node_id], queue='flowpilot')
        return True
    ...
    def force_alter_node(cls, tenant_id: str, ticket_id: str, operator_id: str, request_data_dict: dict) -> bool:
        ...
        # send notification
        from tasks import flow_notification_task
        flow_notification_task.apply_async(args=[tenant_id, ticket_id, target_node_id], queue='flowpilot')
    ...
    def handle_ticket_edge_action(cls, tenant_id: str, ticket_id: str, operator_id: str, request_data_dict:dict) -> bool:
        ...
        if edge_record.type == "reject":
            from tasks import flow_action_hook_task
            transaction.on_commit(lambda: flow_action_hook_task.apply_async(args=[tenant_id, ticket_id, "reject"], queue='flowpilot'))
        if len(next_ticket_node_result_list) == 1 and next_ticket_node_result_list[0].get("type") == "end":
            from tasks import flow_action_hook_task
            transaction.on_commit(lambda: flow_action_hook_task.apply_async(args=[tenant_id, ticket_id, "finish"], queue='flowpilot'))

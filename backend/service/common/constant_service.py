...
class ConstantService(BaseService):
    ...
    def __init__(self):
        ...
        self.PARTICIPANT_TYPE_HOOK = 10  # hook方式，当工单状态叨叨处理人类型配置为kook的状态时，flowpilot将触发一个hook请求，被请求方可以执行有些自动化操作然后回调flowpilot,
        ...
        self.FIELD_TYPE_USERNAME = 60  # 用户名，前端展现时需要调用方系统获取用户列表。flowpilot只保存用户名
        self.FIELD_TYPE_MULTI_USERNAME = 70  # 多选用户名,多人情况逗号隔开，前端展现时需要调用方系统获取用户列表。flowpilot只保存用户名
        self.FIELD_TYPE_ATTACHMENT = 80  # 附件，多个附件使用逗号隔开。调用方自己实现上传功能，flowpilot只保存文件路径
